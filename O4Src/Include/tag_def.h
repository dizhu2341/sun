...
///域名：总长  数据类型：整数
const int TAG_TOTAL_LEN = 34 ; //总长,大块传送协议

///域名：每块大小  数据类型：整数
const int TAG_PIECES_SIZE = 35 ; //每块大小,大块传送协议
...
///域名：压缩算法编号  数据类型：整数
const int TAG_COMPRESS_ID = 43 ; //msgbody数据的压缩算法，０：不压缩
...
///域名：压缩数据比例  数据类型：整数
const int TAG_COMMPRESS_RATE = 86 ; //解压缩时,为了快速获得内存大小,增加一个表示压缩比例的TAG,如果报文中没有出现该字段,必须按以前的模式处理,逐步扩大解压缩缓存
